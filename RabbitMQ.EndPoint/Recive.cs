...
{
    internal interface IRecive
    {
        void ReciveCreateExcel();
    }

    internal class Recive : IRecive, IDisposable
    {
        ...
        private void getExcel(int excelId, int searachId, string address, string prefixFileName)
        {
            using (var scope = Services.CreateScope())
            {
                ...
                var getEntities = scope.ServiceProvider.GetRequiredService<IGetEntitiesByFilterService>();

                var fileName = worker.GetExcelFile<object>(getEntities.Execute(searachId).Data.ToList(), address, prefixFileName);
